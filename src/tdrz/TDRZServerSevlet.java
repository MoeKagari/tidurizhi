...
public class TDRZServerSevlet extends ProxyServerServlet {
	...
	public static void main(String[] args) {
		try {
			new TDRZServerSevlet(new ServerConfig(() -> 2222, () -> false, () -> "127.0.0.1", () -> 1080)).start();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private class TDRZApiHandler extends CommunicationHandler {
		...
		public void onSuccess(HttpServletRequest httpRequest, HttpServletResponse httpResponse, Map<String, String> headers, ByteArrayOutputStream requestBody, ByteArrayOutputStream responseBody) throws IOException {
			CommunicationData data = new CommunicationData(this.time, this.serverName, this.uri, headers, requestBody, responseBody);
			String filename = "json\\" + data.getTime() + "_" + FunctionUtils.notNull(data.getType(), DataType::toString, "undefined");
