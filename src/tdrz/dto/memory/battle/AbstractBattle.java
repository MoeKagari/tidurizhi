...
public abstract class AbstractBattle extends BattleDto {
	...
	public AbstractBattle(JsonObject json) {
		...
		if (json.containsKey("api_escape_idx")) {
			FunctionUtils.forEach(JsonUtils.getIntArray(json, "api_escape_idx"), index -> this.fDeck.escapes.add(index - 1));
		}
		if (json.containsKey("api_escape_idx_combined")) {
			FunctionUtils.forEach(JsonUtils.getIntArray(json, "api_escape_idx_combined"), index -> this.fDeckCombine.escapes.add(index - 1));
		}
	}
