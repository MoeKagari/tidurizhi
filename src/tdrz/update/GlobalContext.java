...
import tdrz.dto.memory.battle.daymidnight.BattleAirbattleDto;
import tdrz.dto.memory.battle.daymidnight.BattleAirbattleLDDto;
import tdrz.dto.memory.battle.daymidnight.BattleDayDto;
import tdrz.dto.memory.battle.daymidnight.BattleMidnightDto;
import tdrz.dto.memory.battle.daymidnight.BattleMidnightSPDto;
import tdrz.dto.memory.battle.daymidnight.CombineBattleAirbattleDto;
import tdrz.dto.memory.battle.daymidnight.CombineBattleAirbattleLDDto;
import tdrz.dto.memory.battle.daymidnight.CombineBattleEachDayDto;
import tdrz.dto.memory.battle.daymidnight.CombineBattleEachDayWaterDto;
...
import tdrz.dto.memory.battle.daymidnight.CombinebattleECDayDto;
import tdrz.dto.memory.battle.daymidnight.CombinebattleECMidnightDto;
import tdrz.dto.memory.battle.daymidnight.CombinebattleMidnightDto;
import tdrz.dto.memory.battle.daymidnight.CombinebattleMidnightSPDto;
import tdrz.dto.memory.battle.info.InfoBattleGobackPortDto;
import tdrz.dto.memory.battle.info.InfoBattleNextDto;
...
import tdrz.dto.memory.battle.info.InfoCombinebattleResultDto;
import tdrz.dto.memory.battle.practice.PracticeBattleDayDto;
import tdrz.dto.memory.battle.practice.PracticeBattleMidnightDto;
import tdrz.dto.memory.battle.practice.PracticeBattleResultDto;
import tdrz.dto.translator.DeckDtoTranslator;
import tdrz.dto.word.AirbaseDto;
...
import tdrz.dto.word.PresetDeckDto;
import tdrz.dto.word.QuestDto;
import tdrz.dto.word.ShipDto;
import tdrz.dto.word.UseItemDto;
import tdrz.gui.window.main.ApplicationMain;
import tdrz.internal.TrayMessageBox;
import tdrz.logic.TimeString;
import tdrz.update.data.ApiData;
import tdrz.update.room.DeckRoom;
import tdrz.update.room.KaisouRoom;
import tdrz.update.room.KdockRoom;
import tdrz.update.room.MainRoom;
import tdrz.update.room.NdockRoom;
import tdrz.update.room.QuestRoom;
...
public class GlobalContext {
	...
	public static void setMaterial(int[] mm) {
		if (currentMaterial != null) {
			int[] material = FunctionUtils.arrayCopy(currentMaterial.getMaterial());
	...
	public static void destroyShip(long time, String event, int id) {
		...
		if (ship != null) {
			...
			FunctionUtils.forEach(ship.getSlots(), item -> destroyItem(time, event, item, count));
			destroyItem(time, event, ship.getSlotex(), -1);

			memoryList.add(new DestroyShipDto(time, event, ship));
			shipMap.remove(ship.getId());
		}
		FunctionUtils.forEach(deckRooms, dr -> FunctionUtils.notNull(dr.getDeck(), deck -> deck.remove(id)));
	}
	...
	public static void updateShip(int id, Consumer<ShipDto> handler) {
		FunctionUtils.notNull(shipMap.get(id), handler);
	}

	public static ShipDto getSecretaryShip() {
		return FunctionUtils.notNull(deckRooms[0].getDeck(), deck -> getShip(deck.getShips()[0]), null);
	}
	...
	public static void update(ApiData data) throws Exception {
		...
		switch (data.getType()) {
			...
			case DESTROYSHIP: {
				...
				GlobalContext.setMaterial(JsonUtils.getIntArray((JsonObject) api_data, "api_material"));
			}
				break;
			case DESTROYITEM: {
				long time = TimeString.getCurrentTime();
				...
				GlobalContext.addMaterial(JsonUtils.getIntArray((JsonObject) api_data, "api_get_material"));
			}
				break;
			...
			case KDOCK_CREATESHIP:
				...
				break;
			case KDOCK_CREATESHIP_SPEEDCHANGE:
				FunctionUtils.forEach(kdockRooms, FunctionUtils.getConsumer(KdockRoom::doCreateshipSpeedchange, data, api_data));
				break;
			case KDOCK_CREATESHIP_GETSHIP:
			...
			case DECK:
				...
				break;
			case DECK_SHIP_LOCK:
				FunctionUtils.forEach(deckRooms, FunctionUtils.getConsumer(DeckRoom::doShipLock, data, api_data));
				break;
			case DECK_CHANGE:
			...
			case MISSION_RETURN:
				break;
			case MISSION_RESULT:
				memoryList.add(new MissionResultDto(Integer.parseInt(data.getField("api_deck_id")), (JsonObject) api_data, TimeString.getCurrentTime()));
				break;
			...
			case REMODEL_SLOT: {
				...
				if (json.containsKey("api_after_slot")) {
					...
					GlobalContext.addNewItem(json.getJsonObject("api_after_slot"));
				}
				if (json.containsKey("api_use_slot_id")) {
					int[] useSlotIds = JsonUtils.getIntArray(json, "api_use_slot_id");
					long time = TimeString.getCurrentTime();
					int group = useSlotIds.length;
					FunctionUtils.forEach(useSlotIds, id -> GlobalContext.destroyItem(time, "改修消耗", id, group));
				}
				GlobalContext.setCurrentMaterial(new MaterialDto(JsonUtils.getIntArray(json, "api_after_material")));
			}
				break;
			...
			case SLOT_ITEM:
				...
				break;
			case SHIP2: {
				mainRoom.doPort(data, data.getJsonObject().get("api_data"));
